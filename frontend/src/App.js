...
function App() {
    ...
    const [wsStatus, setWsStatus] = useState('Отключено');
        ...
        ws.current.onmessage = (event) => {
            ...
            if (data.action === 'create') {
                ...
                setTasks(prevTasks => {
                    ...
                    if (taskExists) {
                        return prevTasks; // Возвращаем прежний список без изменений
                    } else {
                        return [...prevTasks, data.task]; // Добавляем новую задачу
                    }
                });
            } 
    ...
    const handleSubmit = async(e) => {
        ...
        try {
            ...
            setTitle('');
            setDescription('');
            // Нет необходимости вызывать fetchTasks(), так как обновления 
            // будут получены через WebSocket
        } catch (error) {
    ...
    const checkEmails = async() => {
        ...
        try {
            ...
            if (response.data.success) {
                console.log(`Получено ${response.data.emails.length} писем`);
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <div className="mb-2">
                WebSocket статус: <span style={{ color: wsStatus === 'Подключено' ? 'green' : 'red' }}>{wsStatus}</span>
            </div>
                    ...
                    <div>
                        ...
                        <ul className="list-disc pl-5">
                            {emails.map((email, index) => (
                                <li key={`email-${index}-${email.date}`} className="mb-1">
